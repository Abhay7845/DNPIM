...
export default function DisplayValidationComponent(props) {
    ...
    const { digit, cond, itemCode, stoneOptionList, setType2option, findingsOption, tegOfItemOption,allDataFromValidation,
...
console.log(allDataFromValidation,"allDataFromValidation from Display VALIDATION")
    useEffect(async () => {

        await axios.get(`${HostManager.mainHostL3}/npim/size/dropdown/${itemCode}`)
            .then((response) => {
    ...
    if (digit === "B" || digit === "C" || digit === "F" || digit === "R" ||
        ...
        return {sizeUMOQty:(sizeUomQuantity && SizeState[0]),
        ...
        if (digit === "0" || digit === "1" || digit === "2" ||
            ...
            (createTegOfItems(feedShowState)) ? tegQuantity = true : Quantity = true;
